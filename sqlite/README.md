...
qemu-system-x86_64 \
	...
	-append "sqlite_qemu-x86_64 vfs.fstab=[ \"initrd0:/:extract::ramfs=1:\" ] -- /chinook.db 'SELECT * FROM Album LIMIT 10'" \
	-initrd ./initrd.cpio
```
...
qemu-system-aarch64 \
	...
	-append "sqlite_qemu-arm64 vfs.fstab=[ \"initrd0:/:extract::ramfs=1:\" ] -- /chinook.db 'SELECT * FROM Album LIMIT 10'" \
	-initrd ./initrd.cpio
```
...
> [!TIP]
> To make the SQLite session interactive, as opposed to running a specific query, simply remove the `SELECT * FROM Album LIMIT 10` part from the `-append` argument. This will allow you to interact with the SQLite database directly using the shell. The argument would be:
> ```bash
> -append "sqlite_qemu-arm64 vfs.fstab=[ \"initrd0:/:extract::ramfs=1:\" ] -- /chinook.db" \
> ```
